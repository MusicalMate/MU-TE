...
import com.example.mute.databinding.FragmentMyPageImageBinding
import com.example.mute.ui.ContentItemClickListener
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch

class MyPageImageFragment : Fragment() {

    private var _binding: FragmentMyPageImageBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentMyPageImageBinding.inflate(layoutInflater, container, false)
    ...
    private fun initAdapter() {
        val clickListener = ContentItemClickListener { contentInfo ->
            val action =
                MyPageImageFragmentDirections.actionMyPageImageFragmentToImageDetailFragment(
        ...
        val popUpClickListener =
            PopUpClickListener { contentId -> viewModel.deleteImage(contentId) }
        myPageAdapter = MyPageAdapter(clickListener, popUpClickListener)
        binding.rvMyPageImage.adapter = myPageAdapter
    }
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.userInfo.collectLatest { userInfo ->
                    myPageAdapter.submitList(userInfo.imageList)
                }
            }
        }
